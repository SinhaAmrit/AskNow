<div class="dark:text-gray-100">
	<span class="w-full text-2xl pl-2 uppercase pb-10">
		...
		Users: 
	</span>@if(session()->has('message')){{ session('message') }} @endif
	<div class="w-full flex pb-4">
		...
		<div class="w-1/6 relative mx-1">
			<button wire:click="deleteUsers" class="animate-pulse block appearance-none w-full bg-red-600 border border-gray-200 text-white h-12 py-1 px-4 pr-8 rounded leading-tight focus:outline-none focus:bg-white focus:border-gray-500 text-sm">Delete</button>
		</div>
	</div>
	@if($users->isNotEmpty())
	<table class="table-auto w-full mb-6">
		...
		<tbody>
			...
			@if($user->role === "DEV") @continue @endif
			<tr>
				...
				@elseif($user->role === "ADMIN") text-blue-500 font-semibold @endif"
				>
					{{ $user->name }}
					@if($user->role === "ADMIN")
