	...
	<x-jet-dropdown align="right" width="48">
		<x-slot name="trigger">
			<button class="flex p-2 items-center rounded-full bg-white shadow text-gray-400 hover:text-gray-700 text-md" wire:click="readNotifications">
				...
				</svg>@if(auth()->user()->unreadNotifications->count()) <span class="absolute mb-2 ml-3 w-3 border-2 border-white h-3 bg-green-500 rounded-full" />@elseif(!Auth::user()->email_verified_at) <span class="absolute mb-2 ml-3 w-3 border-2 border-white h-3 bg-red-500 rounded-full" /> @endif
			</button>
		</x-slot>
		<x-slot name="content">
